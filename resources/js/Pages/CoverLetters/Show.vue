...
const form = useForm({
  title: props.coverLetterTemplate?.title ?? '',
  body: props.coverLetterTemplate?.body ?? '',
});

const submit = () => {
  form.post(route('cover-letter.update', props.coverLetterTemplate.id), {
    onSuccess: () => {
      form.reset();
    },
  });
...
const removeCoverLetter = () => {
  if (confirm('Are you sure you want to delete this cover letter?')) {
    form.delete(route('cover-letter.destroy', props.coverLetterTemplate.id));
  }
};
</script>
      ...
      <div class="max-w-4xl bg-neutral-800 p-6 rounded-lg shadow-lg">
        ...
        <form @submit.prevent="submit" class="space-y-6 mt-8">
          ...
          <div class="flex justify-end gap-4">
            <SecondaryButton @click.prevent="removeCoverLetter">
              Remove
            </SecondaryButton>
            <PrimaryButton
              type="submit"
